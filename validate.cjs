...
assert.ok(html.includes('https://wa.me/5491134843333'));
assert.ok(html.includes('mailto:administracion@dauja.com.ar'));
assert.ok(!html.includes('-34.7050932'),'Remove outdated map coordinates');
assert.ok(!html.includes('↗'),'Arrows must use SVG rather than emoji glyphs');
assert.ok(!css.includes('↗'),'Mobile navigation arrows must also use SVG');
assert.equal((html.match(/class="client-card"/g)||[]).length,24,'All supplied brands are present');
assert.ok(html.includes('Servicios y flota'));
