...
function renderSeo(config) {
  ...
  const organization = {'@type':'Organization','@id':url('#organization'),name:config.name,legalName:config.legalName,url:base,description:config.description,logo:{'@type':'ImageObject',url:url('assets/logos/dauja-oficial-square.png'),width:512,height:512},areaServed:{'@type':'Country',name:'Argentina'},location:{'@type':'Place',hasMap:config.mapsUrl}};
  if(Number.isFinite(config.latitude) && Number.isFinite(config.longitude)) organization.location.geo = {'@type':'GeoCoordinates',latitude:config.latitude,longitude:config.longitude};
